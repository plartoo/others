...
class SymSpell():
    """
    This is the Python version of SymSpell [https://github.com/wolfgarbe/SymSpell/tree/master/SymSpell] ported by
    'ne3x7' [https://github.com/ne3x7/pysymspell/blob/master/symspell.py], and adapted by me for personal use.
    Original SymSpell has MIT License, and I provide the same to this version.
    ...
    def load_dictionary(self, corpus, encoding='utf-16'):
    ...
    def create_dictionary(self, corpus, encoding='utf-16'):
        ...
        Args:
            corpus (str): Path to corpus file.
        """
        # one char; n/a; abc+numeric; all numeric
        if os.path.exists(corpus):
            with open(corpus, 'r', encoding=encoding) as f:
                for line in f:
                    for key in line.split():
                        ...
                        if key.lower() == 'n/a':
                            ...
                            self._create_dictionary_entry(key.lower(), 1)

        if self._deletes is None:
            self._deletes = dict()

    def write_dictionary(self, outfile, encoding='utf-16'):
        """
