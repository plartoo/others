...
def main():
    ...
    json_activity = {'typeProperties': {'extendedProperties':
                                            {'source_container': 'pacing-report', # required
                                             ...
                                             'header_rows_to_skip': '19', # optional
                                             'footer_rows_to_skip': '6', # optional
    ...
    archive_blob_location = json_activity['typeProperties']['extendedProperties']['zip_archive_path']
    ...
    for blob in blob_content_generator:
        if fnmatch.fnmatch(blob.name, blob_file_name_with_path):
            ...
            # 6. download from the blob and unzip them to destination unzipped folder
            download_blob_file_to_local_folder(block_blob_service, container_name,
                                               blob.name, local_zip_file_name_with_path)
            unzip_file(local_zip_file_name_with_path, dest_folder_for_unzipped_files)

            # 7. Iterate through unzipped files
            for xlsx_f in list_files_in_folder(dest_folder_for_unzipped_files):
                if custom_processing_module is not None:
                    # 7a. if we have additional processing module loaded, we use it here
                    df = custom_processing_module.process_data(xlsx_f)
                else:
                    # 7b. if no custom processing is required, simply read from the local Excel file
                    df = read_excel_file(xlsx_f,
                ...
                local_csv_file_name_with_path = os.path.join(local_output_dir_name, local_csv_file_name)

                # 7c. write the resulting (processed) data frame to csv
            ...
            # 9. move source blob (zip file) to archive folder
