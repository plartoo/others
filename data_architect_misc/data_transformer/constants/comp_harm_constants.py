PROCESSED_DATE_COLUMN = 'PROCESSED_DATE'
YEAR_COLUMN = 'HARMONIZED_YEAR'
MONTH_COLUMN = 'HARMONIZED_MONTH'
DATE_COLUMN = 'HARMONIZED_DATE'
REGION_COLUMN = 'HARMONIZED_REGION'
COUNTRY_COLUMN = 'HARMONIZED_COUNTRY'
ADVERTISER_COLUMN = 'HARMONIZED_ADVERTISER'
MEDIA_TYPE_COLUMN = 'HARMONIZED_MEDIA_TYPE'
CURRENCY_COLUMN = 'HARMONIZED_CURRENCY'
GROSS_SPEND_COLUMN = 'HARMONIZED_GROSS_SPEND'
CATEGORY_COLUMN = 'HARMONIZED_CATEGORY'
SUBCATEGORY_COLUMN = 'HARMONIZED_SUBCATEGORY'
RAW_SUBCATEGORY_COLUMN = 'RAW_SUBCATEGORY'
RAW_BRAND_COLUMN = 'RAW_BRAND'
RAW_SUBBRAND_COLUMN = 'RAW_SUBBRAND'
RAW_PRODUCT_NAME_COLUMN = 'RAW_PRODUCT_NAME'
RAW_MEDIA_TYPE_COLUMN = 'RAW_MEDIA_TYPE'
PRODUCT_NAME_COLUMN = 'HARMONIZED_PRODUCT_NAME'
NOT_AVAILABLE = 'Not Available'

EXPECTED_COLUMNS = [
    # Standard column names we use in competitive harmonization project
    PROCESSED_DATE_COLUMN,
    YEAR_COLUMN,
    MONTH_COLUMN,
    DATE_COLUMN,
    REGION_COLUMN,
    COUNTRY_COLUMN,
    ADVERTISER_COLUMN,
    MEDIA_TYPE_COLUMN,
    CURRENCY_COLUMN,
    GROSS_SPEND_COLUMN,
    CATEGORY_COLUMN,
    RAW_SUBCATEGORY_COLUMN,
    RAW_BRAND_COLUMN,
    RAW_SUBBRAND_COLUMN,
    RAW_PRODUCT_NAME_COLUMN,
    SUBCATEGORY_COLUMN,
    PRODUCT_NAME_COLUMN
]

# Below are the regions, countries, categories, media types
# and globally competing advertisers that we support as of April, 2020.
# Whenever we come across new data items for constants below, add them.
REGIONS = {'Africa-Eurasia', 'Asia Pacific', 'Europe', 'Latin America', 'North America'}

COUNTRIES = {
    'Argentina', 'Australia', 'Austria',
    'Bahrain', 'Belgium', 'Brazil',
    'Canada', 'Chile', 'China', 'Colombia', 'Costa Rica', 'Czech Republic',
    'Denmark', 'Dominican Republic',
    'Ecuador', 'El Salvador', 'Estonia',
    'Finland', 'France',
    'Germany', 'Greece', 'Guatemala',
    'Honduras', 'Hong Kong', 'Hungary',
    'India', 'Indonesia', 'Ireland', 'Italy', 'Israel',
    'Kazakhstan', 'Kenya', 'Kuwait',
    'Latvia', 'Lithuania',
    'Malaysia', 'Mexico', 'Morocco',
    'Netherlands', 'New Zealand', 'Nicaragua', 'Norway',
    'Oman',
    'Pan Arab', 'Pan Asian', 'Panama', 'Paraguay', 'Peru', 'Philippines', 'Poland', 'Portugal', 'Puerto Rico',
    'Qatar',
    'Romania', 'Russia',
    'Saudi Arabia', 'Singapore', 'Slovakia', 'South Africa', 'Spain', 'Sweden', 'Switzerland',
    'Taiwan', 'Thailand', 'Turkey',
    'Ukraine', 'United Arab Emirates', 'United Kingdom', 'Uruguay', 'USA',
    'Venezuela', 'Vietnam'
}

MEDIA_TYPES = {'Cinema', 'Digital', 'Door drops', 'In-store', 'OOH', 'Print', 'Radio', 'TV'}

GLOBAL_COMPETE_ADVERTISERS = {
    # This list is used only as a reference for new team members
    # It is not used in the code.
    'BEIERSDORF',
    'CHURCH & DWIGHT',
    'COLGATE-PALMOLIVE',
    'GENOMMA LAB',
    'GSK',
    'HENKEL',
    'JOHNSON & JOHNSON',
    'LOREAL',
    'P&G',
    'PHILIPS',
    'RECKITT BENCKISER',
    'SANOFI',
    'S.C. JOHNSON',
    'THE CLOROX COMPANY',
    'UNILEVER'
}

CATEGORIES = {'Home Care', 'Oral Care', 'Other', 'Personal Care', 'Pet Nutrition', NOT_AVAILABLE}

ENGLISH_MEDIA_TYPE_MAPPINGS = {
    "(?i)CINEMA.*": "Cinema",
    "(?i)MAGAZINE.*": "Print",  "(?i)NEWSPAPER.*": "Print",  "(?i)PRINT.*": "Print", "(?i)PRESS.*": "Print", "(?i)MG.*": "Print", "(?i)NP.*": "Print",
    "(?i)OUTDOOR.*": "OOH", "(?i)OOH.*": "OOH", "(?i)OUT.*OF.*HOME.*": "OOH",
    "(?i)RADIO.*": "Radio", "(?i)RD.*": "Radio",
    "(?i)TV.*": "TV", "(?i)Television.*": "TV", "(?i)SPOTS": "TV", "FTA.*": "TV", "(?i)CABLE.*": "TV", "(?i)CB.*": "TV",
    "(?i)DIGITAL.*": "Digital", "(?i)INTERNET.*": "Digital", "(?i)ONLINE.*": "Digital", "(?i)MOBILE.*": "Digital",
}

SPANISH_MEDIA_TYPE_MAPPINGS = {
    "(?i)PUBLICIDAD\sEXTERIOR.*": "OOH",
    "(?i)Revista.*": "Print", "(?i)PRENSA.*": "Print",
}

MEDIA_TYPE_MAPPINGS = dict(ENGLISH_MEDIA_TYPE_MAPPINGS, **SPANISH_MEDIA_TYPE_MAPPINGS)

COUNTRY_MAPPINGS = {
    # Mapping between raw country names in regular expression to standardized country names
    "(?i)BAHRAIN.*": "Bahrain",
    "(?i)KAZAK.*": "Kazakhstan",
    "(?i)KAZAKHSTAN.*": "Kazakhstan",
    "(?i)KENYA.*": "Kenya",
    "(?i)KUWAIT.*": "Kuwait",
    "(?i)KSA.*": "Saudi Arabia",
    "(?i)MOROCCO.*": "Morocco",
    "(?i)MORROCO.*": "Morocco",
    "(?i)MORROCCO.*": "Morocco",
    "(?i)OMAN.*": "Oman",
    "(?i)PAN.*ARAB.*": "Pan Arab",
    "(?i)PAN.*ASIA.*": "Pan Asian",
    "(?i)QATAR.*": "Qatar",
    "(?i)RUSSIA.*": "Russia",
    "(?i)SOUTH.*AFRICA.*": "South Africa",
    "(?i)TURKEY.*": "Turkey",
    "(?i)UKRAINE.*": "Ukraine",
    "(?i)UNITED ARAB EMIRATES.*": "United Arab Emirates",
    "(?i)UAE.*": "United Arab Emirates"
}

ADVERTISER_MAPPINGS = {
    # Mapping between raw advertiser names in regular expression to standardized advertiser names
    # This list will grow to a big one and must be kept maintained/updated constantly.
    "(?i)BDF.*": "BEIERSDORF",
    "(?i).*BEIERSDORF.*": "BEIERSDORF",
    "(?i).*BIERSDORF.*": "BEIERSDORF",
    "(?i).*COLGATE.*": "COLGATE-PALMOLIVE",
    "(?i).*CLOROX.*": "THE CLOROX COMPANY",
    "(?i)^CP$": "COLGATE-PALMOLIVE",
    "(?i)GENOMMA.*": "GENOMMA LAB",
    "(?i).*GLAXO.*": "GSK",  # also catches "\u200EGlaxoSmithKline"
    "(?i)^GSK.*": "GSK",
    "(?i).*HENKEL.*": "HENKEL",
    "(?i).*JOHNSON.*&.*JOHNSON.*": "JOHNSON & JOHNSON",
    "(?i)J.*&.*J.*": "JOHNSON & JOHNSON",
    "(?i).*L'?OREAL.*": "LOREAL",  # also catches 'LOREAL'
    "(?i).*PHILIPS.*": "PHILIPS",
    "(?i).*PROCTER.*&.*GAMBLE.*": "P&G",
    "(?i).*PROCTER.*AND.*GAMBLE.*": "P&G",
    "(?i)P.*&.*G.*": "P&G",
    "(?i).*RECKIT(T)?.*": "RECKITT BENCKISER",
    "(?i)^RB\\s*?$": "RECKITT BENCKISER",
    "(?i)^RB\\sAG$": "RECKITT BENCKISER",
    "(?i)SANOFI.*": "SANOFI",
    "(?i)S(.)?C(.)? JOHNSON.*": "JOHNSON & JOHNSON",
    "(?i).*UNILEVER.*": "UNILEVER"
}

ENGLISH_CATEGORY_MAPPINGS = {
    "(?i)^HC$": "Home Care",
    "(?i)HOME.*": "Home Care",
    "(?i).*HOME.*CARE.*": "Home Care",
    "(?i).*HOUSEHOLD.*": "Home Care",
    "(?i).*Laundry.*": "Home Care",
    "(?i)Cleaning.*agent.*": "Home Care",
    "(?i).*Detergent.*": "Home Care",

    "(?i)^OC$": "Oral Care",
    "(?i).*ORAL.*": "Oral Care",
    "(?i).*ORAL.*CARE.*": "Oral Care",
    "(?i).*Dental.*": "Oral Care",
    "(?i).*Dentifrices.*": "Oral Care",
    "(?i).*Mouthwash.*": "Oral Care",
    "(?i).*Toothpaste.*": "Oral Care",
    "(?i).*Toothbrush.*": "Oral Care",

    "(?i).*BABY.*CARE.*": "Personal Care",
    "(?i).*SKIN.*CARE.*": "Personal Care",
    "(?i)Bath.*": "Personal Care",
    "(?i).*BEAUTY.*": "Personal Care",
    "(?i).*BODY.*CARE.*": "Personal Care",
    "(?i)Conditioner.*": "Personal Care",
    "(?i).*Cosmetics.*": "Personal Care",
    "(?i).*CREMA.*PIEL.*": "Personal Care",
    "(?i).*Deodorant.*": "Personal Care",
    "(?i).*DEPILATOR.*": "Personal Care",
    "(?i).*DIAPER.*": "Personal Care",
    "(?i)Face.*Care.*": "Personal Care",
    "(?i).*Facial.*": "Personal Care",
    "(?i)Feminine.*Care.*": "Personal Care",
    "(?i).*Hair.*Care.*": "Personal Care",
    "(?i)Hair.*Colour.*": "Personal Care",
    "(?i)Hair.*Shampoo.*": "Personal Care",
    "(?i).*Hair.*Removal.*": "Personal Care",
    "(?i)Hair.*Styl.*": "Personal Care",
    "(?i)Hair.*Treatment.*": "Personal Care",
    "(?i)Hand.*Wash.*": "Personal Care",
    "(?i).*HIGIENE.*FEMENIN*": "Personal Care",
    "(?i).*HYGIENE.*": "Personal Care",
    "(?i).*Moisturizer.*": "Personal Care",
    "(?i).*MOUTH.*CARE.*": "Personal Care",
    "(?i)^PC$": "Personal Care",
    "(?i)PERSONAL.*CARE.*": "Personal Care",
    "(?i).*SOAP.*": "Personal Care",
    "(?i).*Shampoo.*": "Personal Care",
    "(?i).*SHAVE.*": "Personal Care",
    "(?i)Shower.*": "Personal Care",
    "(?i).*Toiletries.*": "Personal Care",

    "(?i).*ANTISEPTIC.*": "Other",
    "(?i).*WHISKY.*": "Other",
    "(?i).*BOUTIQUE.*": "Other",
    "(?i).*KETCHUP.*": "Other",
    "(?i)(\b)?OTHER(\b)?": "Other",
    "(?i).*Accessories.*": "Other",
    "(?i).*Agriculture.*": "Other",
    "(?i).*Alcohol.*": "Other",
    "(?i)(\b)?ALL.*OTHER.*": "Other",
    "(?i).*Automation.*": "Other",
    "(?i).*Automotive.*": "Other",
    "(?i).*Banking.*": "Other",
    "(?i).*Beverage.*": "Other",
    "(?i).*Building.*": "Other",
    "(?i).*CEREAL.*": "Other",
    "(?i).*Cough.*": "Other",
    "(?i).*Clothing.*": "Other",
    "(?i).*Computers.*": "Other",
    "(?i).*Conglomerate.*": "Other",
    "(?i).*Corporate.*": "Other",
    "(?i).*Dairy.*": "Other",
    "(?i).*Drinks.*": "Other",
    "(?i).*Durables.*": "Other",
    "(?i).*Education.*": "Other",
    "(?i).*Equipments.*": "Other",
    "(?i).*Electrical.*": "Other",
    "(?i).*Event.*": "Other",
    "(?i).*Finance.*": "Other",
    "(?i).*FOOD.*": "Other",
    "(?i).*Fuel.*": "Other",
    "(?i).*HARINA.*": "Other",
    "(?i).*HIGIENE.*PRODUCTO.*INFANTILE.*": "Other",
    "(?i).*HOTEL.*": "Other",
    "(?i).*Internet.*": "Other",
    "(?i).*Investment.*": "Other",
    "(?i).*LACTEO.*": "Other",
    "(?i).*MEDIUM.*": "Other",
    "(?i).*PICK.*UP.*": "Other",
    "(?i).*MINI.*": "Other",
    "(?i).*PREMIUM.*": "Other",
    "(?i).*SMALL.*": "Other",
    "(?i).*UTILITY.*": "Other",
    "(?i).*Land.*": "Other",
    "(?i).*Materials.*": "Other",
    "(?i).*Medicated.*": "Other",
    "(?i).*Medicine.*": "Other",
    "(?i).*Miscellaneous.*": "Other",
    "(?i).*Muscle.*": "Other",
    "(?i).*Office.*": "Other",
    "(?i).*Petroleum.*": "Other",
    "(?i).*Pesticide.*": "Other",
    "(?i).*Pharmaceutical.*": "Other",
    "(?i).*Pharmacy.*": "Other",
    "(?i).*Retail.*": "Other",
    "(?i).*SERVICIO.*BANCARIO.*": "Other",
    "(?i).*SERVICIO.*PUBLICO.*": "Other",
    "(?i).*TEA.*": "Other",
    "(?i).*Telecom.*": "Other",
    "(?i).*Textiles.*": "Other",
    "(?i).*Tobacco.*": "Other",
    "(?i).*Tourism.*": "Other",
    "(?i).*Transport.*": "Other",
    "(?i).*Vitamin.*": "Other",

    "(?i)Pet\\s.*": "Pet Nutrition"
}

SPANISH_CATEGORY_MAPPINGS = {
    "(?i).*BUCAL.*": "Oral Care",

    "(?i).*DETERGENTE.*": "Home Care",
    "(?i).*PRODUCTO.*LIMPIEZA.*DOMESTICA.*": "Home Care",
    "(?i).*BLANQUEADOR.*": "Home Care",
    "(?i).*QUITAMANCHA.*": "Home Care",
    "(?i).*LIMPIADOR.*": "Home Care",
    "(?i).*PRODUCTO.*HOGAR.*": "Home Care",
    "(?i).*SUAVIZANTE.*": "Home Care",
    "(?i).*QUITAGRASA.*": "Home Care",
    "(?i).*ESCOBA.*": "Home Care",
    "(?i).*TOALLA.*PAPEL.*": "Home Care",
    "(?i).*PAPEL.*HIGIENICO.*": "Home Care",
    "(?i).*HIGIENE.*HOGAR.*": "Home Care",
    "(?i).*LAVAPLATO.*": "Home Care",
    "(?i).*SERVILLETA.*": "Home Care",
    "(?i).*JABON.*BARRA.*": "Home Care",

    "(?i).*COLEGIO.*": "Other",
    "(?i).*PASAJERO.*": "Other",
    "(?i).*INMOBILIARI(A)?.*": "Other",
    "(?i).*COOPERATIV.*": "Other",
    "(?i).*GRANO.*": "Other",
    "(?i).*CDT.*": "Other",
    "(?i).*AUTOMOTOR.*": "Other",
    "(?i).*ENLATADO.*": "Other",
    "(?i).*MAYONESA.*": "Other",
    "(?i).*NUTRICION.*": "Other",
    "(?i).*PLASTICO.*": "Other",
    "(?i).*(\b)?DISCO(\b)?.*": "Other",
    "(?i).*ANTICARDIACO.*": "Other",
    "(?i).*OBESIDAD.*": "Other",
    "(?i).*ADMINISTRATIVO.*": "Other",
    "(?i).*PASTA.*": "Other",
    "(?i).*OPTICA.*": "Other",
    "(?i).*JALEA.*": "Other",
    "(?i).*AZUCAR.*": "Other",
    "(?i).*ANTIDIARREICO.*": "Other",
    "(?i).*ACEITUNA.*": "Other",
    "(?i).*AUTOSERVICIO.*": "Other",
    "(?i).*BEBIDA.*": "Other",
    "(?i).*CALZADO.*": "Other",
    "(?i).*CEREAL.*": "Other",
    "(?i).*CREDITO.*": "Other",
    "(?i).*DEBITO.*": "Other",
    "(?i).*COMERCIO.*": "Other",
    "(?i).*GUBERNAMENTALES.*": "Other",
    "(?i).*DEPORTES.*": "Other",
    "(?i).*DIVERSION.*": "Other",
    "(?i).*ENTIDAD.*FINANCIERA.*": "Other",
    "(?i).*EMPRESA.*": "Other",
    "(?i).*HARINA.*": "Other",
    "(?i).*HOTEL.*": "Other",
    "(?i).*INSECTICIDA.*": "Other",
    "(?i).*LACTEO.*": "Other",
    "(?i).*MUEBLE.*": "Other",
    "(?i).*PRODUCTO.*FARMACEUTICO.*": "Other",
    "(?i).*SOPA.*": "Other",
    "(?i).*SERVICIO.*": "Other",
    "(?i).*ALIMENTOS.*": "Other",
    "(?i).*COMPUTACION.*": "Other",
    "(?i).*PUBLICIDAD\sCOMPARTIDA.*": "Other",
    "(?i).*INSTITUCIONAL.*": "Other",
    "(?i).*INSTITUTO.*": "Other",
    "(?i).*MENUDEO.*": "Other",
    "(?i).*MOTOCICLETA.*": "Other",
    "(?i).*NO\sDISPONIBLE.*": "Other",
    "(?i).*ROPA.*": "Other",
    "(?i).*PONQUE.*": "Other",
    "(?i).*SEGURO.*": "Other",
    "(?i).*TELEMERCADEO.*": "Other",
    "(?i).*TIENDA.*": "Other",
    "(?i).*EDUCACION.*": "Other",
    "(?i).*PANIFICACION.*": "Other",
    "(?i).*AGRICULTURA.*": "Other",
    "(?i).*AGROPECUARI.*": "Other",
    "(?i).*CAJA.*COMPENSACION.*": "Other",
    "(?i).*CAJERO.*": "Other",
    "(?i).*PAUTA.*": "Other",
    "(?i).*VEHICULO.*": "Other",
    "(?i).*SOFTWARE.*": "Other",
    "(?i).*HARDWARE.*": "Other",
    "(?i).*DULCE.*": "Other",
    "(?i).*MANTENIMIENTO.*": "Other",
    "(?i).*REPARACION.*": "Other",
    "(?i).*RESTAURANTE.*": "Other",
    "(?i).*GASOLIN.*": "Other",
    "(?i).*GASEOSA.*": "Other",
    "(?i).*FINANCI.*": "Other",
    "(?i).*PINTURA.*": "Other",
    "(?i).*IMPERMEAB.*": "Other",
    "(?i).*TELEVISION.*": "Other",
    "(?i).*UNIVERSIDAD.*": "Other",
    "(?i).*COLCHON.*": "Other",
    "(?i).*PELICULA.*": "Other",
    "(?i).*PERIODICO.*": "Other",
    "(?i).*ESCUELA.*": "Other",
    "(?i).*LLANTA.*": "Other",
    "(?i).*SUERO.*": "Other",
    "(?i).*INVERSION.*": "Other",
    "(?i).*VIAJE.*": "Other",
    "(?i).*BANCO.*": "Other",
    "(?i).*BANCA.*": "Other",
    "(?i).*RADIO.*": "Other",
    "(?i).*CEMENTO.*": "Other",
    "(?i).*PROMOCION.*": "Other",
    "(?i).*AEREA.*": "Other",
    "(?i).*BOLSA.*": "Other",
    "(?i).*FINANCIA.*": "Other",
    "(?i).*DISCOTECA.*": "Other",
    "(?i).*INDUSTRIA.*": "Other",
    "(?i).*COMUNIC.*": "Other",
    "(?i).*RELOJ.*": "Other",
    "(?i).*REPOSTERIA.*": "Other",
    "(?i).*GIMNASIO.*": "Other",
    "(?i).*REVISTA.*": "Other",
    "(?i).*DOMESTIC.*": "Other",
    "(?i).*TEATRO.*": "Other",
    "(?i).*ALARMA.*": "Other",
    "(?i).*LIBRO.*": "Other",
    "(?i).*LIBRERIA.*": "Other",
    "(?i).*CURSO.*": "Other",
    "(?i).*POSTRE.*": "Other",
    "(?i).*SUPOSITORIO.*": "Other",
    "(?i).*CONFERENCIA.*": "Other",
    "(?i).*DISTRIBUIDOR.*": "Other",
    "(?i).*PRIMA.*": "Other",
    "(?i).*COMPUTO.*": "Other",
    "(?i).*UNGUENTO.*": "Other",
    "(?i).*CERVEZA.*": "Other",
    "(?i).*LAPTOP.*": "Other",
    "(?i).*FRUTA.*": "Other",
    "(?i).*VERDURA.*": "Other",
    "(?i).*MAPA.*": "Other",
    "(?i).*PORTAFOLIO.*": "Other",
    "(?i).*VOLTAJE.*": "Other",
    "(?i).*TELEFON.*": "Other",
    "(?i).*GALERIA.*": "Other",
    "(?i).*CONDOMINIO.*": "Other",
    "(?i).*AVION.*": "Other",
    "(?i).*CHOCOLATE.*": "Other",
    "(?i).*JOYA.*": "Other",
    "(?i).*VINO.*": "Other",
    "(?i).*MULTIFUNCIONAL.*": "Other",
    "(?i).*FUNERA.*": "Other",
    "(?i).*ACEITE.*COMESTIBLE.*": "Other",
    "(?i).*LUBRICANTE.*": "Other",
    "(?i).*CASA.*CAMBIO.*": "Other",
    "(?i).*CONSTRUC.*": "Other",
    "(?i).*LECHE.*POLVO.*": "Other",
    "(?i).*ESTUFA.*": "Other",
    "(?i).*TEQUILA.*": "Other",
    "(?i).*HERRAMIENTA.*": "Other",
    "(?i).*CLINICA.*": "Other",
    "(?i).*CAFE.*": "Other",
    "(?i).*CENTRO.*COMERCIAL.*": "Other",
    "(?i).*SALSA.*": "Other",
    "(?i).*LOTERIA.*": "Other",
    "(?i).*REFRESCO.*": "Other",
    "(?i).*SALON.*": "Other",
    "(?i).*TERRENO.*": "Other",
    "(?i).*SALUD.*": "Other",
    "(?i).*AGUA.*": "Other",
    "(?i).*OFICINA.*": "Other",
    "(?i).*PEGAMENTO.*": "Other",
    "(?i).*MUSEO.*": "Other",
    "(?i).*ESTADIO.*": "Other",
    "(?i).*REFRIGERADOR.*": "Other",
    "(?i).*BIZCOCHO.*": "Other",
    "(?i).*CUPON.*": "Other",
    "(?i).*TORTILLA.*": "Other",
    "(?i).*TUBERIA.*": "Other",
    "(?i).*CONDIMENTO.*": "Other",
    "(?i).*ESPECTACULO.*": "Other",
    "(?i).*CLOSET.*": "Other",
    "(?i).*AGENCIA.*": "Other",
    "(?i).*VELA.*": "Other",
    "(?i).*ACCESORIO.*": "Other",
    "(?i).*GANADO.*": "Other",
    "(?i).*GANADERIA.*": "Other",
    "(?i).*MINISTERIO.*": "Other",
    "(?i).*TELA.*": "Other",
    "(?i).*HUEVO.*": "Other",
    "(?i).*PAPELERIA.*": "Other",
    "(?i).*POLITIC.*": "Other",
    "(?i).*PESCADO.*": "Other",
    "(?i).*PENSION.*": "Other",
    "(?i).*PIROTECNICO.*": "Other",
    "(?i).*EXPECTORANTE.*": "Other",
    "(?i).*GOTA.*OFTALMICA.*": "Other",
    "(?i).*ALIANZA.*": "Other",
    "(?i).*COMIDA.*": "Other",
    "(?i).*POSTPAGO.*": "Other",
    "(?i).*SEGURIDAD.*": "Other",
    "(?i).*BIOENERGETIC.*": "Other",
    "(?i).*COSTURA.*": "Other",
    "(?i).*MUSIC.*": "Other",
    "(?i).*ALMACEN.*": "Other",
    "(?i).*CONDON.*": "Other",
    "(?i).*CONVENCION.*": "Other",
    "(?i).*ANUNCIO.*": "Other",
    "(?i).*CAMPANA.*": "Other",
    "(?i).*CAMPAÑA.*": "Other",
    "(?i).*HELAD.*": "Other",
    "(?i).*FARMACEUTIC.*": "Other",
    "(?i).*LENTE.*CONTACTO.*": "Other",
    "(?i).*LENCERIA.*": "Other",
    "(?i).*ENERGIZANTE.*": "Other",
    "(?i).*LECHE.*": "Other",
    "(?i).*ASESOR.*": "Other",
    "(?i).*ANTIACIDO.*": "Other",
    "(?i).*DECORA.*": "Other",
    "(?i).*MENSAJE.*": "Other",
    "(?i).*EMBUTIDO.*": "Other",
    "(?i).*SONIDO.*": "Other",
    "(?i).*SANDALIA.*": "Other",
    "(?i).*TALLER.*": "Other",
    "(?i).*YOGURT.*": "Other",
    "(?i).*FERIA.*": "Other",
    "(?i).*CLUB.*": "Other",
    "(?i).*FUNDACION.*": "Other",
    "(?i).*JEEP.*": "Other",
    "(?i).*BICICLETA.*": "Other",
    "(?i).*APLICACION.*MOVIL.*": "Other",
    "(?i).*ARROZ.*": "Other",
    "(?i).*LEASING.*": "Other",
    "(?i).*GALLETA.*": "Other",
    "(?i).*CARNE.*": "Other",
    "(?i).*MANTEQUILLA.*": "Other",
    "(?i).*ANTIGRIPAL.*": "Other",
    "(?i).*DESCONGESTION.*": "Other",
    "(?i).*REHABILITA.*": "Other",
    "(?i).*AHORRO.*": "Other",
    "(?i).*QUESO.*": "Other",
    "(?i).*DROGUERIA.*": "Other",
    "(?i).*PAN.*": "Other",
    "(?i).*VETERINARI.*": "Other",
    "(?i).*RELIGIO.*": "Other",
    "(?i).*SNACK.*": "Other",
    "(?i).*ANALGESICO.*": "Other",
    "(?i).*CLASIFICAD.*": "Other",
    "(?i).*MODIFICADOR.*": "Other",
    "(?i).*CARGA.*": "Other",
    "(?i).*COMPOTA.*": "Other",
    "(?i).*FERTILIZANTE.*": "Other",
    "(?i).*GAFA.*": "Other",
    "(?i).*PARQUE.*": "Other",
    "(?i).*ENDULZANTE.*": "Other",
    "(?i).*CORTINA.*": "Other",
    "(?i).*PREPAGO.*": "Other",
    "(?i).*GOBIERNO.*": "Other",
    "(?i).*COMBUSTIBLE.*": "Other",
    "(?i).*NATURISTA.*": "Other",
    "(?i).*AGRICOLA.*": "Other",
    "(?i).*BIEN.*RAI.*": "Other",
    "(?i).*MUNICIPAL.*": "Other",
    "(?i).*POLLO.*": "Other",
    "(?i).*MERCADO.*": "Other",
    "(?i).*CAMION.*": "Other",
    "(?i).*JUGO.*": "Other",
    "(?i).*REMESA.*": "Other",
    "(?i).*HATCHBACK.*": "Other",
    "(?i).*INFUSION.*": "Other",
    "(?i).*(\b)?VAN(S)?(\b)?.*": "Other",
    "(?i).*(\b)?TE(\b)?.*": "Other", 
    "(?i).*(\b)?RON(\b)?.*": "Other",
    "(?i).*(\b)?SAL(\b)?.*": "Other",
    "(?i).*JUEGO.*ELECTRONICO.*": "Other",
    "(?i).*FLOR.*": "Other",
    "(?i).*CUERO.*": "Other",
    "(?i).*IMPRESORA.*": "Other",
    "(?i).*PUBLICIDAD.*": "Other",
    "(?i).*PISO.*": "Other",
    "(?i).*TRANSFERENCIA.*FONDO.*": "Other",
    "(?i).*CASA.*": "Other",
    "(?i).*CINTA.*ADHESIVA.*": "Other",
    "(?i).*HIGIENE.*ANIMAL.*": "Other",
    "(?i).*VENTA.*": "Other",
    "(?i).*APERITIVO.*": "Other",
    "(?i).*MEDICAMENTO.*": "Other",
    "(?i).*MEDIO.*": "Other",
    "(?i).*DEPORTIVO.*": "Other",
    "(?i).*VIDRIO.*": "Other",
    "(?i).*GUARDERIA.*": "Other",
    "(?i).*TABLET.*": "Other",
    "(?i).*ANDAMIO.*": "Other",
    "(?i).*DIGESTIVO.*": "Other",
    "(?i).*LABORATORIO.*": "Other",
    "(?i).*ANTICONCEPTIVO.*": "Other",
    "(?i).*TRAJE.*BAÑO.*": "Other",
    "(?i).*TRAJE.*BANO.*": "Other",
    "(?i).*ANTIHISTAMINICO.*": "Other",
    "(?i).*SASTRERIA.*": "Other",
    "(?i).*CUCHILL.*": "Other",
    "(?i).*NASAL.*": "Other",
    "(?i).*ADEREZO.*": "Other",
    "(?i).*EDITORIAL.*": "Other",
    "(?i).*FRIJOL.*": "Other",
    "(?i).*(\b)?MANI(\b)?.*": "Other",
    "(?i).*(\b)?TOS(\b)?.*": "Other",
    "(?i).*(\b)?ATUN(\b)?.*": "Other",
    "(?i).*CLORO.*": "Other",
    "(?i).*ABOGAD.*": "Other",
    "(?i).*PREMIO.*": "Other",
    "(?i).*FUEGO.*ARTIFICIAL.*": "Other",
    "(?i).*ADELGAZANT.*": "Other",
    "(?i).*VINAGRE.*": "Other",
    "(?i).*ZAPATO.*": "Other",
    "(?i).*LABORATORIO.*": "Other",
    "(?i).*(\b)?PILA(\b)?.*": "Other",
    "(?i).*DEPORTIV.*": "Other",
    "(?i).*PROFESIONAL.*": "Other",
    "(?i).*IMPRENTA.*": "Other",
    "(?i).*CHICLE.*": "Other",
    "(?i).*CONSOME.*": "Other",
    "(?i).*CIRUJANO.*": "Other",
    "(?i).*MACROBIOTIC.*": "Other",
    "(?i).*BARRA.*NUTRITIV.*": "Other",
    "(?i).*CENTRO.*DROGA.*": "Other",
    "(?i).*SITIO.*": "Other",
    "(?i).*NATILLA.*": "Other",
    "(?i).*CONSULTORI.*": "Other",
    "(?i).*TATUAJE.*": "Other",
    "(?i).*MIEL.*": "Other",
    "(?i).*CALVICIE.*": "Other",
    "(?i).*QUIROPRACTICO.*": "Other",
    "(?i).*HIGADO.*": "Other",
    "(?i).*MARGARINA.*": "Other",
    "(?i).*HOSPITAL.*": "Other",
    "(?i).*SIROPE.*": "Other",
    "(?i).*ADUANA.*": "Other",
    "(?i).*JOYERIA.*": "Other",
    "(?i).*PAPA.*TOSTADA.*": "Other",
    "(?i).*FARMACIA.*": "Other",
    "(?i).*HEMORROIDE.*": "Other",
    "(?i).*PRESERVATIVO.*": "Other",
    "(?i).*AUTOMOTRIZ.*": "Other",
    "(?i).*PASABOCA.*": "Other",
    "(?i).*SAZON.*": "Other",
    "(?i).*GIRO.*": "Other",
    "(?i).*INFORME.*": "Other",

    "(?i).*ALIMENTO.*PERRO.*": "Pet Nutrition",
    
    "(?i).*COSMETICO.*": "Personal Care",
    "(?i).*COSMETICO.*": "Personal Care",
    "(?i).*RINSE.*": "Personal Care",
    "(?i).*CHAMPU.*": "Personal Care",
    "(?i).*CREMAS.*PIEL.*": "Personal Care",
    "(?i).*HIGIENE.*PERSONAL.*": "Personal Care",
    "(?i).*HIGIENE.*INTIMA.*FEMENINA.*": "Personal Care",
    "(?i).*CREMA.*TRATAMIENTO.*": "Personal Care",
    "(?i).*TRATAMIENTO.*CABELL.*": "Personal Care",
    "(?i).*PROTECCION.*FEMENINA.*": "Personal Care",
    "(?i).*HIGIENE.*PRODUCTO.*INFANTIL.*": "Personal Care",
    "(?i).*FRAGANCIA.*": "Personal Care",
    "(?i).*FIJADOR.*CABELLO.*": "Personal Care",
    "(?i).*PRODUCTO.*CABELLO.*": "Personal Care",
    "(?i).*DESODORANTE.*": "Personal Care",
    "(?i).*ACNE.*": "Personal Care",
    "(?i).*PANALES.*": "Personal Care",
    "(?i).*PAÑAL.*": "Personal Care",
    "(?i).*TINTE.*CABELLO.*": "Personal Care",
    "(?i).*CELULITI.*": "Personal Care",
    "(?i).*RASURADOR.*": "Personal Care",
    "(?i).*JABON.*TOCADOR.*": "Personal Care",
    "(?i).*CREMA.*MANO.*": "Personal Care",
    "(?i).*CREMA.*CUERPO.*": "Personal Care",
    "(?i).*CREMA.*PIERNA.*": "Personal Care",
    "(?i).*TRATAMIENTO.*CABELLO.*": "Personal Care",
    "(?i).*ANTIBACTERIAL.*": "Personal Care",
    "(?i).*CAPILAR.*": "Personal Care",
    "(?i).*JABON.*LIQUIDO.*": "Personal Care",
    "(?i).*AFEITA.*": "Personal Care",
    "(?i).*JABON.*INTIMO.*": "Personal Care",
    "(?i).*JABON.*BANO.*": "Personal Care",
    "(?i).*JABON.*BAÑO.*": "Personal Care",
    "(?i).*MAQUILLAJE.*": "Personal Care",
    "(?i).*TOALLA.*HIGIENICA.*": "Personal Care",
    "(?i).*TOALLA.*SANITARIA.*": "Personal Care",
    "(?i).*CREMA.*MANCHA.*": "Personal Care",
    "(?i).*VARICE.*": "Personal Care",
    "(?i).*PERFUME.*": "Personal Care",
    "(?i).*TOALL.*HUM.*": "Personal Care",
    "(?i).*DRY.*CLEAN.*": "Personal Care",
    "(?i).*DENTIFRICO.*": "Personal Care",
    "(?i).*ANTIMICOTICO.*": "Personal Care",
    "(?i).*ACONDICIONADOR.*": "Personal Care",
    "(?i).*TAMPON.*": "Personal Care",
    "(?i).*DELINEADOR.*": "Personal Care",
    "(?i).*BLOQUEADOR.*": "Personal Care",
    "(?i).*LOCION.*": "Personal Care",
}

CATEGORY_MAPPINGS = dict(SPANISH_CATEGORY_MAPPINGS, **ENGLISH_CATEGORY_MAPPINGS)

MONTH_REFERENCE_BY_LANGUAGE = {
     "Ene": "January",
     "Feb": "February",
     "Mar": "March",
     "Abr": "April",
     "May": "May",
     "Jun": "June",
     "Jul": "July",
     "Ago": "August",
     "Sep": "September",
     "Oct": "October",
     "Nov": "November",
     "Dic": "December",

     "Enero": "January",
     "Febrero": "February",
     "Marzo": "March",
     "Abril": "April",
     "Mayo": "May",
     "Junio": "June",
     "Julio": "July",
     "Agosto": "August",
     "Septiembre": "September",
     "Octubre": "October",
     "Noviembre": "November",
     "Diciembre": "December",
}
