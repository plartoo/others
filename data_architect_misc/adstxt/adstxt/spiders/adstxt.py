...
class AdsTxtSpider(scrapy.Spider):
    ...
    DELIMITER = ','
    OUTPUT_DIR = None
    # _stop_following_links = True # TODO: remove. Does NOT work
    ...
    def load_urls_to_crawl(self):
        cur_dir_path = os.path.dirname(os.path.realpath(__file__))
        url_file = os.path.join(cur_dir_path, 'urls_to_scrape_2.csv')
    ...
    def start_requests(self):
        ...
        for url in urls:
            print('start:', url)
            # Alternative way to stop redirects: https://doc.scrapy.org/en/latest/topics/downloader-middleware.html#module-scrapy.downloadermiddlewares.redirect
        ...
        # if response.url != :
            ...
            for line in response.body.split(b'\n'):
                ...
                if split_line[0].strip():
                    ...
                    yield ads_txt_dict
        else: #TODO: remove this because it creates empty rows in CSV file
            print('SKIPPED:', response.url, ':', response.status)
            yield {}
